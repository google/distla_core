...
def _overlap_matrix_invsqrt_part2(
  ...
  second part.
  """
  invsqrt_kwargs = dict(invsqrt_kwargs)
  overlap_matrix = _set_padded_diagonal(overlap_matrix, k)
  _, om_invsqrt, _, _ = invsqrt.invsqrt(overlap_matrix, **invsqrt_kwargs)
...
def overlap_matrix_invsqrt(
  ...
  logging.info("Distributing S")
  orig_shape = overlap_matrix.shape
  overlap_matrix = _pad_for_distribution(overlap_matrix, orig_shape)
  overlap_matrix = pops.distribute(overlap_matrix)
  overlap_matrix.block_until_ready()  # For benchmarking
  ...
  om_invsqrt = _overlap_matrix_invsqrt_part2(
      ...
      p_sz,
      precision,
      # Working around pmap not liking dicts.
...
def compute_energy_weighted_density_matrix(
  ...
  logging.info("Distributing H")
  objective_fn = _pad_for_distribution(objective_fn, objective_fn.shape)
  objective_fn = pops.distribute(objective_fn)
  logging.info("Distributing DM")
  density_matrix = _pad_for_distribution(density_matrix, density_matrix.shape)
  density_matrix = pops.distribute(density_matrix)
  ...
  logging.info("Undistributing EDM")
  en_weighted_density_matrix = pops.undistribute(
      en_weighted_density_matrix,
      host_local=True,
      collect_to_host=True,
  )
  # REDACTED We return here the padded density matrix. It's easier to remove
  # the padding later when gathering results, but this isn't the prettiest way
  # of doing things.
...
@functools.partial(pops.pmap, static_broadcasted_argnums=(2,), out_axes=None)
def compute_ebs(objective_fn, density_matrix, dtype):
  local_sum = jnp.sum(
      objective_fn.astype(dtype).conj() * density_matrix.astype(dtype))
...
def purify_density_matrix(
  ...
  ebs = compute_ebs(objective_fn, density_matrix, np.float64)
  del objective_fn
  ...
  density_matrix.block_until_ready()  # For benchmarking
  logging.info("Undistributing the density matrix")
  density_matrix = pops.undistribute(
      density_matrix,
      host_local=True,
      collect_to_host=True,
  )
