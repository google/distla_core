...
class ComplexDeviceArray:
    ...
    def __add__(self, other):
        if not isinstance(other, ComplexDeviceArray):
            raise TypeError(
                "ComplexDeviceArray cannot be added to object of "
                f"type {type(other)}"
            )
        return ComplexDeviceArray(
    ...
    def conj(self):
        # NOTE : is it save to pass by reference?
...
def moveaxis(tensor, src, dest):
    if isinstance(tensor, debug_array.DebugArray):
        return tensor.moveaxis(src, dest)
...
def tensordot(a, b, axes, precision=jax.lax.Precision.DEFAULT):
    if isinstance(a, ComplexDeviceArray) or isinstance(b, ComplexDeviceArray):
        real = jnp.tensordot(
            a.real, b.real, axes, precision=precision
        ) - jnp.tensordot(a.imag, b.imag, axes, precision=precision)
        imag = jnp.tensordot(
            a.imag, b.real, axes, precision=precision
        ) + jnp.tensordot(a.real, b.imag, axes, precision=precision)
        return ComplexDeviceArray(real, imag)
...
def einsum(subscripts, *operands, precision=jax.lax.Precision.DEFAULT):
    if len(operands) != 2:
        raise ValueError(
            f"only two operands allowed in einsum, "
    ...
    if isinstance(operands[0], ComplexDeviceArray) and isinstance(
        ...
        real = jnp.einsum(
            subscripts, operands[0].real, operands[1].real, precision=precision
        ) - jnp.einsum(
            subscripts, operands[0].imag, operands[1].imag, precision=precision
        )
        imag = jnp.einsum(
            subscripts, operands[0].real, operands[1].imag, precision=precision
        ) + jnp.einsum(
            subscripts, operands[0].imag, operands[1].real, precision=precision
        )
        return ComplexDeviceArray(real, imag)
...
def dot(a, b, precision=jax.lax.Precision.HIGHEST):
    ...
    """
    if isinstance(a, ComplexDeviceArray) or isinstance(b, ComplexDeviceArray):
        real = jnp.dot(a.real, b.real, precision=precision) - jnp.dot(
            a.imag, b.imag, precision=precision
        )
        imag = jnp.dot(a.imag, b.real, precision=precision) + jnp.dot(
...
def kron(a, b):
    if isinstance(a, ComplexDeviceArray) or isinstance(b, ComplexDeviceArray):
        real = jnp.kron(a.real, b.real) - jnp.kron(a.imag, b.imag)
        imag = jnp.kron(a.imag, b.real) + jnp.kron(a.real, b.imag)
        return ComplexDeviceArray(real, imag)
    return jnp.kron(a, b)


def eye(N, M=None, k=0, dtype=None, arithmetic_dtype=complex):
    if arithmetic_dtype is complex:
        identity = jnp.eye(N, M, k, dtype)
        return ComplexDeviceArray(identity, jnp.zeros_like(identity))

    return jnp.eye(N, M, k, dtype)
