...
import asic_la.testutils as testutils
...
def test_jit_mul():
    ...
    for v in [5, -2.2, -2.5j, 2 + 2.5j]:
        ...
        np.testing.assert_allclose(
            (a * v).real, fn_static(A, v).real, rtol=1e-5
        )
        np.testing.assert_allclose(
            (a * v).imag, fn_static(A, v).imag, rtol=1e-5
        )
...
def test_basic_truediv():
    ...
    for v in [5, -2.2, -2.5j, 2 + 2.5j]:
        np.testing.assert_allclose(
            (a / v).real, (A / v).real, atol=1e-7, rtol=1e-6
        )
        np.testing.assert_allclose(
            (a / v).imag, (A / v).imag, atol=1e-7, rtol=1e-6
        )
...
def test_tensordot():
    ...
    B = cw.ComplexDeviceArray(b.real, b.imag)
    AB = cw.tensordot(
        A, B, ([1, 2], [3, 4]), precision=jax.lax.Precision.HIGHEST
    )
    ab = jnp.tensordot(
        a, b, ([1, 2], [3, 4]), precision=jax.lax.Precision.HIGHEST
    )
...
def test_array(dtype):
    np.random.seed(0)
    shape = (2,) * 5
    a = np.random.random_sample(shape).astype(
        dtype
...
def test_einsum():
    ...
    B = cw.ComplexDeviceArray(b.real, b.imag)
    AB = cw.einsum(
        "abcde,aBcDe ->bBdD", A, B, precision=jax.lax.Precision.HIGHEST
    )
    ab = cw.einsum(
        "abcde,aBcDe ->bBdD", a, b, precision=jax.lax.Precision.HIGHEST
    )
...
def test_sqrt():
    ...
    shape = (2,) * 5
    a = (
        np.random.random_sample(shape) + 1j * np.random.random_sample(shape)
    ).astype(np.complex64)
    A = cw.ComplexDeviceArray(a.real, a.imag)
    sqrtA = cw.sqrt(A)
    sqrta = cw.sqrt(a)
    eps = 10 * testutils.eps(jax.lax.Precision.HIGHEST, np.float32)
    np.testing.assert_allclose(sqrtA.real, sqrta.real, atol=eps, rtol=eps)
    np.testing.assert_allclose(sqrtA.imag, sqrta.imag, atol=eps, rtol=eps)
...
def test_kron():
    a = np.random.normal(size=(2, 2)) + np.random.normal(size=(2, 2)) * 1j
    b = np.random.normal(size=(3, 4)) + np.random.normal(size=(3, 4)) * 1j
    A = cw.ComplexDeviceArray(a.real, a.imag)
    B = cw.ComplexDeviceArray(b.real, b.imag)
    AB = cw.kron(A, B)
    ab = jnp.kron(a, b)
...
@pytest.mark.parametrize("M", [None, 4])
@pytest.mark.parametrize("k", [0, 1])
@pytest.mark.parametrize("dtype", [jnp.float32, jnp.complex64])
def test_eye(N, M, k, dtype):
    A = cw.eye(N, M, k, dtype)
    a = jnp.eye(N, M, k, dtype)
    np.testing.assert_allclose(A.real, a.real)
    np.testing.assert_allclose(A.imag, a.imag)
