...
# =============================================================================
"""Tests forShardedDiscretedProbabilityFunction"""
import functools
import jax
...
from asic_la.sharded_probability_function import sharded_discrete_probability_function as sqwf
from asic_la.sharded_probability_function import complex_workaround as cw


class ShardedDiscretedProbabilityFunctionTest(unittest.TestCase):
    def test_dot_free_only(self):
        np.random.seed(3)
        val = np.random.uniform(size=(2,) * 10 + (8, 128)) + 1.0
        building_block = np.random.uniform(size=(128, 128)) + 1.0
        state = sqwf.ShardedDiscretedProbabilityFunction(
            val, perm=tuple(range(20)), num_global_discretes=0
        )
        axes = (2, 4, 1, 0, 5, 3, 6)

        new_state = state.discrete_dot(building_block, axes)
        expected = np.tensordot(
            val,
            building_block.reshape((2,) * 14),
            [axes, list(range(len(axes), 2 * len(axes)))],
        )
        expected = np.moveaxis(expected, list(range(-len(axes), 0)), axes)
        ...
        assert new_state.perm == axes + tuple(range(7, 20))
    ...
    def test_dot_double_left_perm_as_tuple(self):
        ...
        axes = (2, 15, 19)
        new_state = state.discrete_dot(
            cw.ComplexDeviceArray(building_block.real, building_block.imag), axes
        )
        expected = np.tensordot(
            val.reshape((2,) * 20),
            building_block.reshape((2,) * 6),
...
def test_pytree_registry():
    ...
    A = sqwf.ShardedDiscretedProbabilityFunction(a, list(range(l)), False)
