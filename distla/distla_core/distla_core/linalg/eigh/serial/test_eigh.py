...
from distla_core.linalg.eigh.serial import eigh
...
def test_positive_projector(N, precision):
  np.random.seed(15)
  H = np.random.randn(N, N).astype(np.float32)
  H = jnp.array(0.5 * (H + H.conj().T))
  ev_exp, _ = jnp.linalg.eigh(H)
  k_expected = np.count_nonzero(ev_exp > 0.)
  P = eigh.positive_projector(H, 0., precision)
...
def test_split_spectrum(N, precision):
  ...
  Hm, Vm, Hp, Vp = eigh._split_spectrum_work(P, H, V, k, precision)
